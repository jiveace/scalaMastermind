...
class MasterMindCodeBreaker() {
  ...
  def nextGuess(guess: List[Int], history: List[(List[Int], (Int, Int))]): List[Int] =
    if (guess == List(0, 0, 0, 0) && history.size > 0) throw new IllegalStateException("guess has looped around to (0,0,0,0)")
  ...
  def breakCodeSeq(lastGuess: List[Int], history: List[(List[Int], (Int, Int))]): List[Int] = {
    ...
    else if (lastGuess == List(0, 0, 0, 0)) nextGuess(incrementGuess(lastGuess), history)
